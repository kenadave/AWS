...
13) For Amazon EC2 Image Builder:
...
14)  If the exam question asks about "feature flags" or "dynamic application settings," pick AppConfig. If it asks about "compliance auditing" or "resource history," pick AWS Config.

15)  AppConfig is for your application's code and features; Config is for your AWS infrastructure.

16)  EC2 Image Builder is a fully-managed service that simplifies customization, testing, distribution, and lifecycle management of Amazon Machine Images (AMIs) and container images.

17)  Use drift detection to detect when resources have been changed outside of CloudFormation.

Use change sets to validate that the intent of a stack update matches the actions that CloudFormation takes when the change set is initiated.

18) Use deployment configurations such as AWS CodeDeploy AppSpec files to define deployment and validation steps.

19) Delve into metrics derived from X-Ray traces, such as latency, request rates, fault rates, and response time distributions, using the service map to monitor application health.

20) Use ServiceLens: Leverage the ServiceLens map for enhanced observability of your services and applications. This allows for integrated viewing of traces, metrics, logs, alarms, and other health information.

21) CloudWatch: Can tell you that Service A has high latency and Service B has high latency. It cannot automatically tell you that Service A is slow because it’s waiting on Service B.
X-Ray: The Service Map is generated by the traces. It shows the physical relationship between components. It visualizes the "blast radius" of a failure in real-time.

22) On the exam, if you see a requirement to monitor "customer experience" or "endpoint availability" when there is low traffic, the answer is almost always CloudWatch Synthetics Canaries.

23) Implement anomaly detection:
